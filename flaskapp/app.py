...
def trainmodel(X, y):
    ...
    imp = IterativeImputer(
        max_iter=10, random_state=0, initial_strategy="mean", add_indicator=True
    )
    ...
    pipe = Pipeline(
        steps=[("coltrans", ct), ("linear", linear_model.LinearRegression())]
    )
...
def plotly_hist(df, prediction=1, degreetype="All", majorfield="All Fields"):
    ...
    plot_series2 = df[(df.CIPDESC_new == majorfield) & (df.CREDDESC == degreetype)][
        "EARN_MDN_HI_2YR"
    ].astype(int)
    ...
    fig.add_trace(
        go.Histogram(
            x=plot_series1, name="All Fields", histnorm="percent", xbins=dict(size=1000)
        )
    )
    fig.add_trace(
        go.Histogram(
            x=plot_series2, name=majorfield, histnorm="percent", xbins=dict(size=1000)
        )
    )
...
def result():
    ...
    p3 = plotly_hist(
        df=df,
        degreetype=variables["CREDDESC"],
        prediction=pred_final,
        majorfield=variables["CIPDESC_new"],
    )
